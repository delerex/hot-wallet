...
class PrivateKeyBase(object):
    ...
    def to_string(self):
        return base58.b58encode_check(bytes.fromhex("80") + self.key)
...
class PublicKeyBase(object):
    ...
    @staticmethod
    def from_bytes(key_bytes: bytes):
    ...
    @staticmethod
    def from_private_key(private_key: PrivateKeyBase):
...
class PublicKey(PublicKeyBase):
    ...
    def address(self, compressed=True, testnet=False):
        ...
        r.update(base)
        checksum = r.digest()[:4]
        encoded = base58.b58encode(base + checksum)
        return "EOS" + encoded
